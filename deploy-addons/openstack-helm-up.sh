...
kubectl apply -f $BOOTKUBE_DIR/deploy-rbac/dev.yaml --validate=false
kubectl apply -f $BOOTKUBE_DIR/deploy-rbac/ceph.yaml --validate=false

### PREPARE DEPENDENCIES:
sudo apt-get install -y python-minimal ceph-common
git clone https://github.com/openstack/openstack-helm.git $BOOTKUBE_DIR/openstack-helm && cd $BOOTKUBE_DIR/openstack-helm && git checkout $OSH_BRANCH
# v1k0d3n: Do we really need a variable below?
if [ ! -e /usr/local/bin/sigil ]; then
...
cd $BOOTKUBE_DIR/openstack-helm/helm-toolkit/utils/secret-generator
./generate_secrets.sh all `./generate_secrets.sh fsid`
cd $BOOTKUBE_DIR/openstack-helm/
helm init
helm serve &
